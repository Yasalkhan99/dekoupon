                    ...
                    <li key={c.id} id={`o-${encodeURIComponent(c.id)}`} className={`flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-md ${viewMode === "grid" ? "items-stretch gap-5 p-6" : "sm:flex-row sm:items-center sm:gap-6 sm:p-6"}`}>
                        ...
                        <div className="flex h-24 w-24 flex-col items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 px-1 text-center text-white shadow-inner sm:h-28 sm:w-28">
                          {badge.type === "text" ? (
                            <>
                              ...
                              {badge.line2 && <span className="mt-0.5 text-[10px] font-semibold leading-tight opacity-95 sm:text-xs">{badge.line2}</span>}
                      ...
                      <div className={`flex min-w-0 flex-1 flex-col items-start p-5 pt-0 ${viewMode === "list" ? "sm:flex-row sm:items-center sm:justify-between sm:pt-5" : "sm:pt-0"}`}>
                        <div className="min-w-0 w-full flex-1 space-y-2 text-left">
                          ...
                          <button type="button" onClick={handleRevealOnly} className="w-full text-left font-bold text-zinc-900 transition hover:text-blue-600 cursor-pointer">
